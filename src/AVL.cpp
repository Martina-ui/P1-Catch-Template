...
void AVLTree::check_commands(string commands) {
    ...
    if (cmd == "insert") {
        ...
        if (!all_of(commandList[1].begin(), commandList[1].end(), [](char c){ return isalpha(c) || c == ' '; })) { //this syntax was generated by codespace autocomplete
            throw invalid_argument("Error: Name must contain only alphabetic characters and spaces");
        }
        if (commandList[2].length() != 8 || !all_of(commandList[2].begin(), commandList[2].end(), ::isdigit)) { //this syntax was generated by codespace autocomplete
...
Node* AVLTree::insertHelper(Node* node, string name, string ufid) { //this was taken from class slides on Balanced BSTs slide on AVL tree: C++ insert
    ...
    //for sytanx for lines 107-142 i used https://www.geeksforgeeks.org/dsa/insertion-in-an-avl-tree/ and the class slides for Balanced BSTs talking about rotations
...
void AVLTree::remove(Node* node, string ufid) { 
// Command format: “remove ID”, where ID is an integer
// Functionality: Find and remove the account with the specified ID from the tree; Balancing is not required after and will not be tested
// Steps
// Ensure the ID exists within the tree (Useful to use search())
// Traverse recursively through the tree to reach the desired node to delete
...
// Make sure to return the correct node (or null) at the end to reconnect subtrees at the ancestor level;
    ...
    if (ufid < node->ufid) {
        remove(node->left, ufid); //syntax from project 1 ppt slides on BST remove
    } else {
...
void AVLTree::inorder_ufid_vector(Node* node, vector<int> inorder_vec) {
    ...
    inorder_vec.push_back(stoi(node->ufid));
